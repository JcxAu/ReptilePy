...
def get_data(url: str, ua: str, ssl: bool = True):
    global state
    try:
        r = requests.get(url, headers=ua, timeout=(4, 5), verify=ssl)
...
def download_img(url: str, ua: str, ssl: bool = True):
    global state
    try:
        data = requests.get(url, headers=ua, timeout=(5, 7), verify=ssl)
    except requests.exceptions.RequestException:
        state = language['请求超时 !']
        return '下载失败'
...
def find_url_algorithm(url: str, ua: str, ssl: bool = True):
    url_l = []
...
def get_date_html(url: str, num_download: int, lock, time_sleep: float = 0.2, ssl: bool = True) -> int:
    ...
    while i < int(num_download):
        ...
        lock.release()
        if reptile_res != '下载失败' and reptile_res != 0:
            download_error += 1
        elif reptile_res == '下载失败':
            download_error += 1
        i += 1
