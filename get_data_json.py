...
def get_json(url: str, ua: dict, ssl: bool = True) -> dict:
    res_d: dict = {'state': None, 'info': None, 'json_data': None}
    try:
        result = requests.get(url, headers=ua, verify=ssl, timeout=(5, 7))
...
def find_url_from_json(url: str, ua: dict, ssl: bool = True) -> dict:
    res_d: dict = {'state': None, 'info': None, 'url': None}
    result = get_json(url, ua, ssl)
    if result['state'] == '成功':
...
def reptile_algorithm(url: str, ua: dict, ssl: bool = True) -> dict:
    ...
    if url != '' and ('http://' in url[:7] or 'https://' in url[:8]):
        img_url_d = find_url_from_json(url, ua, ssl)
        if img_url_d['state'] == '成功':
            download_info =  download_img(img_url_d['url'], ua, ssl)
            ...
            res_d['state'] = '失败'
            log.write_log(language_d['图片请求失败 !'] + str(img_url_d['info']) + '\nurl: ' + str(url))
            return res_d
...
def get_date_json(url: str, num_download: int, lock, time_sleep: float = 0.2, ssl: bool = True) -> dict:
