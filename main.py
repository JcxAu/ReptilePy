...
import get_data_html, change_ini, get_data_img, language, get_data_json
...
def save_mode(mode: str):
    ...
    # 改变标签内容
    if mode == 'HTML':
        word = language_d['目前的下载设置为: 通过图片所在的网页获取']
    elif mode == 'IMG':
        word = language_d['目前的下载设置为: 通过图片自身的网址获取']
    elif mode == 'JSON':
        word = language_d['目前的下载设置为: 通过返回的 JSON 对象获取图片']
    else:
        word = language_d['暂无']
...
def reptile_img(url: str, download_num: int, lock_n, ssl: bool = True, time_sleep: float = 0.2):
    info = get_data_img.get_date_img(url, download_num, lock_n, time_sleep, ssl)
...
def reptile_html(url: str, download_num: int, lock_n, ssl: bool = True, time_sleep: float = 0.2):
    info = get_data_html.get_date_html(url, download_num, lock_n, time_sleep, ssl)
    download_info.put(info)

# 使用 JSON 模式的爬虫


def reptile_json(url: str, download_num: int, lock_n, ssl: bool = True, time_sleep: float = 0.2):
    info = get_data_json.get_date_json(url, download_num, lock_n, time_sleep, ssl)
    download_info.put(info)
...
def reptile_thread(url: str, download_num: int, mode: str, name: str, time_sleep: float = 0.2, ssl: bool = True):
    if mode == 'HTML':
        ...
        thread = threading.Thread(target=reptile_json, args=(url, download_num, lock, ssl, time_sleep), name=name, daemon=True)
        thread.start()
...
def reptile_threads(url: str):
    ...
    if info_l[1] == 'HTML' or info_l[1] == 'IMG' or info_l[1] == 'JSON':
        if info_l[0] <= 18:
            for i in range(0, info_l[0]):
                reptile_thread(url, 1, info_l[1], 'reptile_' + str(i), 0.5, info_l[2])
        else:
            num = int(info_l[0] / 18)
            residue = info_l[0] - num * 18
            for i in range(0, residue):
                reptile_thread(url, num + 1, info_l[1], 'reptile_' + str(i), info_l[2])
            for i in range(residue, 18):
                reptile_thread(url, num, info_l[1], 'reptile_' + str(i), 0.5, info_l[2])
    else:
...
def download_mode():
    ...
    if not if_download:
        if not mode_win_open:  # 确保只能开启一个窗口
            ...
            # 俩个选项
            ra_but = tk.Radiobutton(mode_win, text=language_d['通过图片所在的网页获取'], variable=s, value='HTML', command=lambda: save_mode(s.get()))
            ra_but.grid(row=1, column=0, padx=r_p_x, pady=r_p_y, sticky='W')
            ra_but1 = tk.Radiobutton(mode_win, text=language_d['通过图片自身的网址获取'], variable=s, value='IMG', command=lambda: save_mode(s.get()))
            ra_but1.grid(row=2, column=0, padx=r1_p_x, pady=r1_p_y, sticky="W")
            ra_but2 = tk.Radiobutton(mode_win, text=language_d['通过返回的 JSON 对象获取图片'], variable=s, value='JSON', command=lambda: save_mode(s.get()))
...
def language_chinese():
    if not if_download:
        ...
        if result != change_ini.language_d['暂无'] and result != 'Chinese':
